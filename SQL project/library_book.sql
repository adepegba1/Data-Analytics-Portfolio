...
USE db_LibraryManagement;
	...
	CREATE TABLE tbl_book (
		...
		book_PublisherName VARCHAR(100) NOT NULL, FOREIGN KEY(book_PublisherName) REFERENCES tbl_publisher(publisher_PublisherName)
	);
	...
	CREATE TABLE tbl_book_loans (
		...
		book_loans_BookID INT NOT NULL,FOREIGN KEY(book_loans_BookID) REFERENCES tbl_book(book_BookID),
		book_loans_BranchID INT NOT NULL, FOREIGN KEY(book_loans_BranchID) REFERENCES tbl_library_branch(library_branch_BranchID),
		book_loans_CardNo INT NOT NULL, FOREIGN KEY(book_loans_CardNo) REFERENCES tbl_borrower(borrower_CardNo),
	...
	CREATE TABLE tbl_book_copies (
		...
		book_copies_BookID INT NOT NULL, FOREIGN KEY(book_copies_BookID) REFERENCES tbl_book(book_BookID) ON UPDATE CASCADE ON DELETE CASCADE,
		book_copies_BranchID INT NOT NULL, FOREIGN KEY(book_copies_BranchID) REFERENCES tbl_library_branch(library_branch_BranchID) ON UPDATE CASCADE ON DELETE CASCADE,
	...
	CREATE TABLE tbl_book_authors (
		...
		book_authors_BookID INT NOT NULL, FOREIGN KEY(book_authors_BookID) REFERENCES tbl_book(book_BookID) ON UPDATE CASCADE ON DELETE CASCADE,
...
FROM tbl_book book
INNER JOIN tbl_book_copies copies
ON book.book_BookID = copies.book_copies_BookID
INNER JOIN tbl_library_branch branch
...
LEFT JOIN tbl_book_loans loans
...
and whose DueDate is 2/2/18, retrieve the book title, the borrower's name, 
and the borrower's address.  */
SELECT  library_branch_BranchName, book_loans_DueDate, book_Title, 
		borrower_BorrowerName, borrower_BorrowerAddress
FROM tbl_library_branch branch
INNER JOIN tbl_book_loans loans
...
INNER JOIN tbl_borrower borrower
ON loans.book_loans_CardNo = borrower.borrower_CardNo
WHERE   branch.library_branch_BranchName = 'Sharpstown' AND
		loans.book_loans_DueDate = '2/2/18';
...
/* #6- Retrieve the names, addresses, and number of books checked out for all borrowers
 ...
 SELECT borrower_BorrowerName, borrower_BorrowerAddress, COUNT(book_loans_CardNo)
 FROM tbl_borrower borrower
 INNER JOIN tbl_book_loans loans
 ON loans.book_loans_CardNo = borrower.borrower_CardNo
...
SELECT library_branch_BranchName, book_Title, SUM(book_copies_No_Of_Copies)
FROM tbl_book book
INNER JOIN tbl_book_authors authors
